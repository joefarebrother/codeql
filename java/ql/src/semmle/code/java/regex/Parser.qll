...
// locations to nodes.
// optimizer assumes that the size of str is 0 which is wrong.
pragma[noopt]
cached
private predicate rowCol(string str, int pos, int row, int col) {
  str instanceof ParsedString and
  pos = 0 and
  row = 0 and
  col = 0
  or
  exists(int r0, int c0, string charat, int prevpos |
    rowCol(str, prevpos, r0, c0) and
    pos = prevpos + 1 and
...
class Node extends TNode {
  ...
  int getEndOffset() { exists(int pos | result = tokenize(text, _, pos, next - 1).length() + pos) }
